...
| Testing and balance framework | `required-mvp` | No direct runtime logic until harnessed | `scripts/balance_report.py`, `main-game/pipeline/tools/build_grain_manifest.py`, `build_choice_catalogue.py`, `simulate_balance.py`, `balance_model.py`, `main-game/draft/releases/planning/balance/`, `main-game/pipeline/releases/release-1-mvp/{grain,balance,reports}/` | `chief_architect`, documentation steward | `py scripts/balance_report.py --release release-1-mvp`; `py main-game/pipeline/tools/simulate_balance.py --release release-1-mvp` | Keep. **Implemented:** static report, grain manifest, choice catalogue import, abstract policy simulator (2026-06-19). Next review: runtime capture P1–P7; refine sim vs playtest deltas. |
