...
| Testing and balance framework | `required-mvp` | No direct runtime logic until harnessed | `scripts/balance_report.py`, `scripts/validation/balance_report_impl.py`, `main-game/pipeline/tools/build_grain_manifest.py`, `main-game/draft/releases/planning/balance/`, `main-game/pipeline/releases/release-1-mvp/grain/`, `main-game/pipeline/releases/release-1-mvp/reports/balance_report.md` | `chief_architect`, documentation steward | `py scripts/balance_report.py --release release-1-mvp`; `py main-game/pipeline/tools/build_grain_manifest.py --release release-1-mvp` | Keep. **Implemented:** static report + grain manifest + seed catalogues (2026-06-19). Next review: choice catalogue, policy simulator, runtime capture matrix P1–P7. |
