...
This document details the architectural updates implemented in the Release 1 sandbox (`main-game/draft/releases/release-1-mvp/`) to support the refined writing gate mechanics, flat inspiration capacity, and separated resource validation. These changes have been implemented in `classes_non_canon.rpy` and `functions_non_canon.rpy` for review by the Chief Architect before promotion to production.
...
**Promotion note:** Add `CONFRONTATION_THRESHOLD` constant and `is_confrontation_ready` method to `PlayerStats` in `main-game/prod-game/game/classes.rpy`.
