...
def write_outputs(args: argparse.Namespace, data: dict[str, Any], slot_routes: dict[str, tuple[str, str, str]], penance_routes: dict[str, tuple[str, str, str]], files: list[Path]) -> None:
    ...
    for outcome in slot_routes:
        if outcome not in call_sites:
            append_gap(gaps, "router_outcome_mismatches", "main-game/prod-game/game/classes.rpy", 0, "StoryState.SLOT_EXIT_ROUTES", f"Outcome {outcome} has no extracted call site.", "Chief Architect", "Confirm unused route is expected.")
...
def main() -> int:
    ...
    parser.add_argument("--out-dir", default=f"main-game/pipeline/releases/{DEFAULT_RELEASE}/graph")
    ...
    slot_routes, penance_routes = extract_routes(ROOT / "main-game" / "prod-game" / "game" / "classes.rpy")
