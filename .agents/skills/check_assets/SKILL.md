...
1. Ensure the production game scripts under `main-game/prod-game/game/` are fully structured.
...
3. **Analyze the Audit Results:**
   ...
   - **Compliance Failure:** If the script outputs `[COMPLIANCE FAILURE]`, inspect the reported list of unmanifested images or audio tracks and add their declarations (using `declare_image_with_fallback` or `register_audio`) to `main-game/prod-game/game/assets_manifest.rpy`.
