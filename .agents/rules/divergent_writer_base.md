...
# Domain: speculative/spec_scripts/ (write), narrative/canon/ + docs/canon/ (read-only)
...
1. **Spec scripts only.** Write to `speculative/spec_scripts/releases/<release>/` using filename `dayrdd_<persona>_spec.rpy` (e.g. `day101_tension_spec.rpy`). Never write `dayrdd_non_canon.rpy` or files under `narrative/writers_room/releases/`.
...
6. **Idea sidecar (required).** After each pass, append to `speculative/idea_archive/releases/<release>/dayrdd_<persona>_ideas.md`:
...
7. **Context firewall.** Do not read `speculative/idea_archive/**` unless the orchestrator explicitly passes a path for "archive mining." Do not read other days' `*_spec.rpy` unless listed in the current task brief. Do not read prior days' `dayrdd_non_canon.rpy` — use the orchestrator-supplied **`continuity_handoff.md`** section for prose continuity.
8. **Voice awareness.** Read relevant `narrative/templates/Voice_Guides/*_voice_guide.md` for characters in scene; your lens may push tone but must not invent new character facts that contradict canon files.
...
- `narrative/writers_room/releases/<release>/story_board.md` (relevant rows)
...
| Artifact | Path |
|----------|------|
| Spec script | `speculative/spec_scripts/releases/<release>/dayrdd_<persona>_spec.rpy` |
| Idea sidecar | `speculative/idea_archive/releases/<release>/dayrdd_<persona>_ideas.md` |
