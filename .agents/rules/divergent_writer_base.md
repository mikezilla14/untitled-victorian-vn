...
# Domain: main-game/pipeline/ (write), main-game/canon/ + main-game/canon/ (read-only)
...
1. **Spec scripts only.** Write to `main-game/pipeline/releases/<release>/` using filename `dayrdd_<persona>_spec.rpy` (e.g. `day101_tension_spec.rpy`). Never write `dayrdd_non_canon.rpy` or files under `main-game/draft/releases/`.
...
6. **Idea sidecar (required).** After each pass, append to `main-game/pipeline/releases/<release>/dayrdd_<persona>_ideas.md`:
...
7. **Context firewall.** Do not read `main-game/pipeline/**` unless the orchestrator explicitly passes a path for "archive mining." Do not read other days' `*_spec.rpy` unless listed in the current task brief. Do not read prior days' `dayrdd_non_canon.rpy` — use the orchestrator-supplied **`continuity_handoff.md`** section for prose continuity.
8. **Voice awareness.** Read relevant `main-game/canon/voice_guides/*_voice_guide.md` for characters in scene; your lens may push tone but must not invent new character facts that contradict canon files.
...
- `main-game/draft/releases/planning/story_board.md` (relevant rows)
...
| Artifact | Path |
|----------|------|
| Spec script | `main-game/pipeline/releases/<release>/dayrdd_<persona>_spec.rpy` |
| Idea sidecar | `main-game/pipeline/releases/<release>/dayrdd_<persona>_ideas.md` |
