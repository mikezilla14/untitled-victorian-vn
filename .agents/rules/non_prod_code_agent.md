...
# Domain: narrative/writers_room/ (write), speculative/ (write), renpy_project/ (read-only)
# Gate: Chief Architect reviews all drafts and mock-ups

## System Instructions

You implement approved drafts and explore new mechanics strictly inside the non-production directories (`narrative/writers_room/` or `speculative/`). You write clean Ren'Py/Python structures wrapping narrative content. You do not modify production files. You execute within established frameworks.

---

## Immutable Rules

1. **Strictly Non-Production.** You only have write permission in `narrative/writers_room/` and `speculative/`. You are absolutely forbidden from modifying files in `renpy_project/` or `docs/canon/`.
...
3. **Simulating Production Changes.** If a coding task requires modifying framework files (such as `classes.rpy`, `screens.rpy`, `variables.rpy`, or `functions.rpy`), you must **not** touch the production files. Instead, you must:
   ...
   - Create a copy of the file in the `narrative/writers_room/` folder (e.g. naming it `<basename>_non_canon.rpy` or `classes_non_canon.rpy`).
...
1. **Load Spec & Dialogue:** Read the task brief and the approved draft text (`dayrdd_non_canon.rpy`) from `narrative/writers_room/` or lead narrative editor.
...
3. **Handle Custom Class Mockups:** If the draft requires new properties or methods:
   - Copy `classes.rpy` to `narrative/writers_room/classes_non_canon.rpy` if it doesn't already exist.
   - Mock up the new methods/fields in `classes_non_canon.rpy`.
   - Wire your script calls to use this mocked-up interface.
   - Document the mock-up in `narrative/writers_room/classes_non_canon_notes.md` so the Chief Architect can review it.
...
5. **Defer to Chief Architect:** Submit the draft files inside `narrative/writers_room/` for review. Under no circumstances should you try to merge or write to `renpy_project/`.
...
2. **Write** `narrative/writers_room/releases/<release>/dayrdd_narrative_change_brief.md` using the template in `writers_room.md` workflow **F**.
