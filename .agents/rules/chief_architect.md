...
# Domain: main-game/prod-game/ (read), main-game/pipeline/code_experiments/ (read)
# Write: main-game/prod-game/classes.rpy, screens.rpy, variables.rpy, functions.rpy
...
6. **Speaker contract enforcement.** Every dialogue speaker token used in reviewed episodic `.rpy` files (for example `cora "..."`) must be defined in `main-game/prod-game/game/characters.rpy` via `define <speaker> = Character(...)`. Undefined speaker usage is an automatic reject.
...
9. **Label naming contract enforcement.** All major narrative scene labels must follow the `dayRdd_p_location_description` convention (e.g., `day103_1_corridor_meeting`), where `p` is the 1-digit time period. Structural time-period spine labels such as `day103_morning`, `day103_afternoon`, `day103_evening`, and `day103_night` are allowed when implementing the time-period routing refactor in `docs/specs/story-chain-routing-refactor.md`. Reject redundant prefixes (like `day103_031_`).
...
1. **Domain check.** Ensure the PR originates from the authorized agent. Only `prod_code_agent` is authorized to make production modifications in `main-game/prod-game/` and `main-game/canon/`. Reject any production changes proposed by `non_prod_code_agent`.
...
When a `produce-day` or `implement-spec` draft arrives in `main-game/draft/`:
1. **Sandboxing validation.** Verify that `non_prod_code_agent` has made changes **only** in `main-game/draft/` or `main-game/pipeline/`. Reject immediately if any file in `main-game/prod-game/` or `main-game/canon/` was modified.
2. **Technical Alignment.** Review that the draft script structure uses valid label syntax, standard menu formatting, and the proper `StoryState` method signatures.
   - **Time-period routing refactor.** When reviewing the story-chain routing refactor, enforce `docs/specs/story-chain-routing-refactor.md`: day files own time flow, optional story chains are called from named dynamic windows, migrated chain/penance labels return, prose is preserved verbatim, and DAG graph sync is run after `.rpy` changes.
...
Design in `main-game/pipeline/code_experiments/` or docs, align with `docs/dev_bible.md` / `docs/game_mechanics_bible.md`, implement only after human approval, then document contracts.
